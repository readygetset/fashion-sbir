...
class GPTDecoder(nn.Module):
    def __init__(self, embed_dim: int, transformer_width: int, transformer_heads: int, transformer_layers: int, vocab_size: int, max_seq_len: int):
        super(GPTDecoder, self).__init__()
        
        self.token_embedding = nn.Embedding(vocab_size, transformer_width)
        self.positional_embedding = nn.Parameter(torch.zeros(1, max_seq_len, transformer_width))
        self.transformer = Transformer(transformer_width, transformer_layers, transformer_heads)
        self.ln_f = LayerNorm(transformer_width)
        self.fc_out = nn.Linear(transformer_width, vocab_size)
    ...
    def forward(self, caption, context):
        # caption: [batch_size, seq_len]
        # context: [batch_size, embed_dim]
        seq_len = caption.size(1)
        caption_emb = self.token_embedding(caption) + self.positional_embedding[:, :seq_len, :]
        caption_emb = caption_emb + context.unsqueeze(1)  # Contextual embedding 추가
        
        x = caption_emb.permute(1, 0, 2)  # [seq_len, batch_size, embed_dim]
        x = self.transformer(x)  # [seq_len, batch_size, embed_dim]
        x = self.ln_f(x.permute(1, 0, 2))  # [batch_size, seq_len, embed_dim]
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            print("isinstance - modified ResNet")
            ...
            print("Is not instance - Visual Transformer")
        ...
        self.decoder = GPTDecoder(
            ...
            max_seq_len=context_length  # 최대 시퀀스 길이 지정
        )
    ...
    def forward(self, image, text, sketch):
        ...
        sketch_features = sketch_features / (sketch_features.norm(dim=-1, keepdim=True) + eps)
